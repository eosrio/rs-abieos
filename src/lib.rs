...
impl AbieosContract {
    ...
    pub fn load_json_file(&mut self, path: &str) -> Result<&mut Self, AbieosError> {
        ...
        match std::fs::read_to_string(path) {
            Ok(file) => match ref_abieos.set_abi_json_native(self.name, &file) {
                Ok(_) => {
                    self.abiLoaded = true;
                    Ok(self)
                }
    ...
    pub fn load_abi(&mut self, abi: AbiLike) -> Result<&mut Self, AbieosError> {
        ...
        let result = match abi {
            AbiLike::Json(abi_json) => ref_abieos.set_abi_json_native(self.name, &abi_json),
            AbiLike::Hex(abi_hex) => ref_abieos.set_abi_hex_native(self.name, &abi_hex),
            AbiLike::Bin(abi_bin) => ref_abieos.set_abi_bin_native(self.name, &abi_bin),
        };
        match result {
            Ok(_) => {
                self.abiLoaded = true;
    ...
    pub fn contract(&self, account_name: NameLike) -> AbieosContract {
        match account_name {
            ...
            NameLike::String(name) => {
                let name_u64 = self
                    .string_to_name(name.as_str())
                    .expect("Failed to convert name to u64");
                AbieosContract::new(self.ctx(), name_u64)
            }
            NameLike::U64(name) => AbieosContract::new(self.ctx(), name),
            NameLike::I32(name) => AbieosContract::new(self.ctx(), name as u64),
        }
    }
}
...
impl Abieos {
    ...
    pub fn c_string_to_name(&self, name: &CStr) -> u64 {
        ...
        unsafe { abieos_string_to_name(ctx, name.as_ptr()) }
    }
    ...
    pub fn get_type_for_action_native(
        &self,
        contract: u64,
        action: u64,
    ) -> Result<String, AbieosError> {
    ...
    pub fn get_type_for_table_native(
        &self,
        contract: u64,
        table: u64,
    ) -> Result<String, AbieosError> {
    ...
    pub fn get_type_for_action_result(
        &self,
        contract: &str,
        action: &str,
    ) -> Result<String, AbieosError> {
    ...
    pub fn abi_bin_to_json(&self, abi: &[u8]) -> Result<String, AbieosError> {
        ...
        unsafe {
            let p = abieos_abi_bin_to_json(ctx, abi_bin_data, abi_bin_size);
            if p.is_null() {
